...
def create_app() -> FastAPI:
    ...
    from deployment_server.utils.fastapi_extra import get_openapi_custom
    from deployment_server.routers import health, project

    server_container = ServerContainer()

    @asynccontextmanager
    async def lifespan(_app: FastAPI):
        server_container.core.init_resources()
        server_container.gateways.init_resources()
        yield
        server_container.core.shutdown_resources()
        server_container.gateways.shutdown_resources()

    app = FastAPI(lifespan=lifespan)
    app.container = server_container
    ...
    async def home():
        return PlainTextResponse(f"This is {server_container.config.core.name()}")
